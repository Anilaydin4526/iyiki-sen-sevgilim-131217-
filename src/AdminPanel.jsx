...
function AdminPanel() {
  ...
  useEffect(() => {
    if (content) {
      ...
      setClosingDraft(content.closingMessage || '');
    }
  }, [content]);
  ...
  return (
    <div className="main-bg">
      ...
      {/* Closing Section with Admin Controls */}
      <div className="admin-section">
        <h2>Kapanış</h2>
        <div className="admin-controls">
          <h3>Kapanış Metni</h3>
          <textarea
            className="edit-textarea-large"
            placeholder="Kapanış yazınızı girin..."
            value={closingDraft}
            onChange={(e) => setClosingDraft(e.target.value)}
          />
          <button
            onClick={() => updateContent({ ...content, closingMessage: closingDraft })}
            className="add-btn"
            style={{ background: '#388e3c', marginTop: 10 }}
          >
            Kaydet
